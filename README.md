...
2D Canvas存在一些缺点影响了在一些场景中的用途，那H5 3D Canvas也就是WebGL就变成一个很好的选择了。
其实现在的H5游戏引擎基本都转向webgl了，在H5游戏方面，2Dcanvas处在逐步被抛弃的阶段。
...
很多前端同学对WebGL的第一反应都是，WebGL开发门槛太高了。用起来很难，还不能调试。
其实对于WebGL，还是建议使用现有的引擎库。通用的引擎库有threeJS，pixi等。很多做webgl的都是，拿threejs改改。
现在VR视频比较火，krpano，UtoVR等是比较专注做VR的引擎。
对于H5游戏，国内比较流行的有白鹭的Egret，还有Layabox和Cocos等。
...
说到WebGL的渲染性能，就得从两个角度来看这个问题。
一个是Opengl的执行性能。WebGL其实就是opengl在Web上的接口。那么它的性能肯定与opengl性能强相关。
另一个就是浏览器的架构损耗，WebGL到真正的opengl中间还隔了一个浏览器，所以浏览器的架构损耗也是需要关注的。
下面我们就分别从这两个角度来看WebGL的性能问题。

#### 5.3 OpenGL（WebGL）流水线

GPU或者opengl是批量处理的，比如它可以同时处理很多像素点的渲染。另外一点opengl是流水线式执行的。就是说前一条指令第一阶段执行完后，开始执行第二阶段时，就再塞一条指令。

所以在写WebGL或者opengl代码的时候得充分利用这一特点。尽量批量处理以及避免打断GPU的执行流水线。

#### 5.4 如何充分利用OpenGL流水线

避免回读帧缓冲区。遇到过一些用例，绘制过程中会去操作画出来的东西的像素值。这样会完全打断GPU的运行效率特别低。仍然这个例子，回读就好比点一个按钮还要把手机拿起来看看按钮上显示的小字。这样就完全破坏批量处理的流程。这点其实不光在webgl中需要注意，在2D canvas中也是一样的，回读效率会特别低。

#### 5.5 浏览器线程同步

![Alt text](/5-1.png)

WebGL的浏览器损耗方面，主要就是要避免线程同步。
原本WebGL的JS调用和真正执行opengl是在两个线程并行执行的。当使用webgl的查询语句去查询一些状态的时候，就会发生一次等待。
所以在使用WebGL要尽量减少这种等待。
...
GPU Driver的bug比较容易出现在非主流的opengl指令或者流程中。

硬件加速场景，也就是使用gpu的场景，比如像WebGL这种深度使用GPU的场合。兼容性问题总是会遇到的。特别是在碎片化很严重的android平台下。
下面给的是android中主流的GPU系列。可以根据这张表用来做覆盖测试。国内主流的GPU厂商有高通，ARM以及Imagination。他们各自在OpengL ES 2.0和Opengl 3.0都有自己的系列GPU。
如果要做非常全面的测试那就得每款GPU每个Driver版本都要过一遍。这样工作量几乎是不可能完成的。那么最简单的覆盖用例就是这6个格子中至少得各选一款对应手机。

关于兼容性问题，其实最容易出兼容性的地方就是一些非主流的opengl或者WebGL指令和流程。
